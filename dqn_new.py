...
from chainer import cuda, optimizers, Variable,Chain,Link,ChainList
...
def main():
	...
	for i in range(1,args.play_number+1) :
		...
		while game_state < 3 and agent.step < 30:
			#今の時間をとっておく
			now_time = time.time()
			#現在の状態を取得
...
class Agent():
	def __init__(self,learning_width, learning_height, interval_w, interval_h):
		...
		self.exploration = 35#εを下げ始めるタイミング
		#その他設定
		self.step = 0#ステップ数	
		self.total_step = 0
		self.goal = 0
	...
	def stock_experience(self,g):
        ...
        '''
        if reward < 0:
            update_step = data_index - 1 + self.data_size
            times = 1  # episode length - time
            while self.D[4][update_step % self.data_size] == 0 and times < 30:  # 30時刻以上前には遡らない
                self.D[2][update_step % self.data_size] += reward * (1.0 / (self.num_of_actions + 1) ** times)  # 遡って価値を追加
                update_step -= 1
                times += 1
        '''
	...
	def Q_forward(self, state, action, reward, state_dash, ep_end):
		...
		max_Q_cpu = chainer.cuda.to_cpu(max_Q_dash)
		#max_Q_cpu = cuda.to_cpu(max_Q_dash)
		target_cpu = chainer.cuda.to_cpu(target)
